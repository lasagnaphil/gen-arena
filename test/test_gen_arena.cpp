...
TEST_CASE("gen_arena_simple_test") {
    ...
    auto [a, a_ptr] = arena.insert(Obj(1));
    REQUIRE(arena.size() == 1);
    auto [b, b_ptr] = arena.insert(Obj(2));
    REQUIRE(arena.size() == 2);
    auto [c, c_ptr] = arena.insert(Obj(3));
    arena.get(b)->set(4);
    ...
    arena.foreach_with_ref([&](GenArena<Obj>::Ref ref, Obj& value) {
